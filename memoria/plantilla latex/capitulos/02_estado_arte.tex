...
autoencoders y el espacio latente. El capítulo termina con una
...
\begin{figure}[htbp]
    ...
    \caption{Envolvente ADSR. Los dos primeros tramos duran un valor fijo. El tercer tramo depende de cuánto tiempo
...
Un autoencoder es una red neuronal entrenada para intentar reconstruir su propia entrada.
Está formado por dos bloques. El codificador, que transforma la entrada en una
representación de dimensión mucho menor; y el decodificador, que intenta reconstruir
la entrada original a partir de esa representación reducida. La representación intermedia
recibe el nombre de código o vector latente. Los autoencoders son capaces de organizar estos
vectores latentes en base a las características de los datos de entrada, de modo que entradas similares acaban
situadas cerca unas de otras en un espacio. Es lo denominado el espacio latente. 
...
se ve forzada a quedarse con los rasgos que de verdad
...
sin que se le haya indicado en ningún momento qué es un armónico o qué es un timbre.

\subsection{El autoencoder variacional}

Un autoencoder convencional organiza el espacio latente sin ninguna garantía
sobre su estructura. Nada impide que queden zonas vacías en este espacio. 
Un punto tomado de una de esas zonas puede decodificarse en algo sin
...
onda del conjunto de entrenamiento, esa falta de predictibilidad es un problema.

El autoencoder variacional o \gls{vae} \cite{Kingma2013} resuelve esa limitación
cambiando la naturaleza del vector latente. En lugar de producir un vector de valores
...
varianzas $\boldsymbol{\sigma}^2$. Así pues, el vector latente que recibe el decodificador se
obtiene muestreando esa distribución. El problema de este muestreo es que, al ser
una operación aleatoria, no es derivable y bloquea la propagación del gradiente durante 
el entrenamiento. Para solucionarlo, Kingma y Welling proponen el truco de reparametrización 
(Ecuación~\eqref{eq:reparam}). El vector latente se expresa como una función 
determinista de la media y la desviación típica, utilizando una muestra de ruido 
externa. De este modo, la aleatoriedad queda aislada en un término independiente 
de la red y el gradiente puede propagarse sin impedimentos.
...
Para evitar que el espacio latente quede lleno de huecos vacíos, a la
función de pérdida se le añade un segundo término: la divergencia de
Kullback-Leibler. Esta métrica compara la distribución del codificador
con una normal estándar y actúa como un imán, obligando a todas las
nubes de probabilidad a agruparse y solaparse ordenadamente en torno
al centro. Así, la pérdida total equilibra la fidelidad de la
reconstrucción con este requisito de regularidad, tal como muestra la
Ecuación~\eqref{eq:vaeloss}.
...
importancia relativa de ambos términos y da nombre a la variante conocida como
$\beta$-VAE \cite{Higgins2017}. Higgins y sus coautores lo introducen como un
hiperparámetro ajustable que equilibra la capacidad del canal latente frente a la
precisión de la reconstrucción, y con valores por encima de la unidad obtienen
representaciones \emph{factorizadas}, en las que cada dimensión del espacio
recoge un único factor de variación de los datos y puede moverse sin alterar el
resto. Sobre imágenes de caras, por ejemplo, una dimensión gobierna el giro de la
cabeza y otra la anchura de la sonrisa.

Conviene no confundir esa propiedad con la continuidad, que es la que interesa
aquí y que aporta el término de divergencia en cualquier autoencoder variacional.
Lo que $\beta$ regula es cuánto pesa la regularización frente a la fidelidad de la
reconstrucción. Valores altos dan un espacio muy regular a costa de
reconstrucciones pobres, porque el modelo prioriza acercar todas las
distribuciones a la normal estándar antes que reproducir cada entrada con
detalle; valores bajos hacen lo contrario. El compromiso adoptado en este trabajo
se justifica en el capítulo~\ref{cap:ml}.

Un fallo habitual en estos modelos es el conocido como colapso posterior.
Ocurre cuando la red ignora por completo el espacio latente y el término 
de divergencia se anula. Este fallo fue observado por Bowman y sus 
coautores \cite{Bowman2016} al aplicar VAEs a texto. Esto pasa porque 
el decodificador es lo suficientemente potente como para reconstruir los 
datos por su cuenta, sin necesitar ayuda del espacio latente. Para solucionarlo, 
en este trabajo se ha aplicado su misma técnica: introducir el peso de la 
divergencia de forma progresiva. Así, durante las primeras épocas el modelo
se centra solo en aprender a reconstruir, y el peso se va subiendo poco a poco
hasta su valor definitivo, tal como se explica en el capítulo~\ref{cap:ml}.
...
aprendizaje de la representación y después de ajuste adversario. Sus autores, Caillon y
...
Es un problema que aparece de nuevo en este trabajo al fijar la dimensión del
latente. Su interés para este trabajo es que demuestra que la síntesis neuronal 
puede alcanzar velocidades de tiempo real, aunque sobre
...
mediante un oscilador convencional, con lo que la carga computacional no 
...
tablas para generar formas de onda de 512 muestras e interpolar entre ellas en
el espacio latente. El resultado se distribuye como \emph{plugin} VST/AU para
\gls{daw}.

Dos observaciones suyas resultan especialmente importantes aquí. La primera es
...
la misma estrategia que se adopta en este trabajo. Conviene subrayar que 
la limitación aparece ya sobre una GPU de sobremesa. Al trabajar sobre 
microcontroladores de bajo coste, la limitación es aún más severa y
obliga a generar las tablas de forma anticipada.
...
plano con el dedo esa limitación es un problema. Esta es una de las razones que justifican
emplear en este proyecto un autoencoder variacional en lugar de uno convencional. Los
...
la variedad de las formas de onda generadas. Su conclusión fue que emplear las 
variables latentes directamente como parámetros de entrada produce una transición 
suave entre las ondas generadas cuando los cambios de esos parámetros son pequeños.
Es exactamente la propiedad sobre la que descansa el control táctil de este instrumento.

Más recientemente, Wavespace \cite{Lee2024} propone un generador de tablas
basado en un autoencoder variacional cuyo espacio latente se divide en subespacios 
asociados a distintos estilos de forma de onda. El generador se condiciona además 
con descriptores tímbricos y morfológicos, de modo que el usuario puede manipular 
cada subespacio de forma independiente en lugar de moverse por un plano indiferenciado.
Los autores lo prototipan también como \emph{plugin} de oscilador para \gls{daw}, y
...
\hypertarget{RE1.1c}{}\label{RE1.1c}
